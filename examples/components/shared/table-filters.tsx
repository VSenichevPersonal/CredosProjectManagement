                ...
                {filter.type === "date" && (
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          ...
                          !activeFilters.get(filter.id) && "text-muted-foreground"
                        )}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {activeFilters.get(filter.id) ? (
                          format(new Date(activeFilters.get(filter.id)), "dd.MM.yyyy", { locale: ru })
                        ) : (
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        mode="single"
                        selected={activeFilters.get(filter.id) ? new Date(activeFilters.get(filter.id)) : undefined}
                        onSelect={(date) => onFilterChange(filter.id, date?.toISOString() || null)}
                        initialFocus
                        locale={ru}
                      />
                    </PopoverContent>
